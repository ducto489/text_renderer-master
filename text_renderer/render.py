...
class Render:
    def __init__(self, cfg: RenderCfg):
        ...
        self.max_text_retries = 3  # Add max retries constant

    @retry(stop=stop_after_attempt(3), retry=retry_if_exception_type(PanicError))
    def __call__(self, *args, **kwargs) -> Tuple[np.ndarray, str]:
        ...
        try:
            ...
            logger.exception(e)
            raise PanicError("Failed to render image") from e

    def gen_single_corpus(self) -> Tuple[PILImage, str, PILImage, PILImage]:
        for attempt in range(self.max_text_retries):
            font_text = self.corpus.sample()
            if font_text and font_text.text and font_text.text.strip():  # Validate text is not empty or just whitespace
                break
            if attempt == self.max_text_retries - 1:
                raise PanicError("Failed to generate valid non-empty text after max retries")
        
        if not font_text or not font_text.text or not font_text.text.strip():
            raise PanicError("Generated text is empty or contains only whitespace")
        ...
        if text_color is None:
            raise PanicError("Failed to generate text color")
        ...
        if text_mask is None:
            raise PanicError("Failed to draw text on background")
    ...
    def paste_text_mask_on_bg(
        ...
        """
        x_offset, y_offset = 0, 0 #utils.random_xy_offset(transformed_text_mask.size, bg.size)
    ...
    def get_text_color(self, bg: PILImage, text: str, font: FreeTypeFont) -> FontColor:
        ...
        r = np.random.randint(0, np.int64(mean * 0.7))
        g = np.random.randint(0, np.int64(mean * 0.7))
        b = np.random.randint(0, np.int64(mean * 0.7))
    ...
    def norm(self, image: np.ndarray) -> np.ndarray:
        ...
        if self.cfg.height != -1 and self.cfg.height != image.shape[0]:
            ...
            width = np.int64(width // (height / self.cfg.height))
            image = cv2.resize(
